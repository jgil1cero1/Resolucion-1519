...
function BuscarNormatividad() {
    ...
    var strurl = location.protocol + "//" + location.host + "/Transparencia/_api/web/lists/getbytitle('Normatividad')/items?$select=Title,LinkFilename,Fecha,FileLeafRef,Clasificac_x00f3_n,A_x00f1_o,Descripci_x00f3_n&$top=2000&$filter=substringof('" + $('#txt_sboxNorma').val() + "',Title) or substringof('" + $('#txt_sboxNorma').val() + "',Fecha)or substringof('" + $('#txt_sboxNorma').val() + "',FileLeafRef) or substringof('" + $('#txt_sboxNorma').val() + "',A_x00f1_o) or substringof('" + $('#txt_sboxNorma').val() + "',Clasificac_x00f3_n) ";
    $.ajax({
        url: strurl,
        method: "GET",
        ...
        success: function (data) {
            ...
            $.each(data.d.results, function (index, item) {
                ...
                var gfreggerwg = "&nbsp; " + item.Clasificac_x00f3_n + "&nbsp; ";
